...
> 文档状态：现行技术设计
>
> 当前实现：MigrationGuard `1.0.5`，最近核对：2026-09-11。
>
> 产品事实与视觉规范分别以
...
- `migration_guard/batch_workflow.py`：更新前资产范围规划、核验结果筛选、资源
  去重、海外 Jira 提交分组和窗口等待。
...
作为源清单快照：“更新并复核”先只读发现全部文件并让用户选择范围，再仅更新、
核验所选文件。迁移直接复用已选范围，迁移前增量检查源提交并快速刷新目标状态，
迁移后只刷新目标文件状态，提交窗口关闭后仅查询快照 revision 之后的源、目标
提交。源 revision 取完整扫描已读取的最高匹配提交，不依赖工作副本根目录的
mixed revision；目标工作区和 externals 上下文在单个迁移阶段内复用。
...
未提交改动；有本地工作区时，用户从同一视图进入现有精确核验。两个入口分别从
最新可见页签向前查找，选择第一张包含对应路线任务
的页签，按钮显示实际命中的页签日期；不能因为最新页签只有纯海外任务而让 Trunk
入口显示空表。固定表加载后必须停留在可多选的单号列表，用户确认“使用选中”后
才锁定后续写操作范围；状态查询自动在后台进行且支持手动刷新，不替换或禁用候选
列表。手动粘贴解析遵循同一流程。Jira 版本登记只显示中性等待态，不得在 SVN
资产证据返回前标绿。所选单号范围继续约束资产选择、工作副本更新和迁移。Trunk
流程以海外 trunk 为完成终点，Trunk-OB 流程以 OSOB 为完成终点。

国内 trunk → 国内 OB 是独立的本地单阶段流程。它不查询海外三阶段远端 URL，也
不依赖飞书海外映射表；在国内 OB 远端 URL 尚未配置前，只使用两套本地工作副本
完成精确更新、核验、迁移、提交和提交后复核。
...
若当前源路径不存在，工具从本任务最后 revision 之后读取模块根路径历史；只有
最后相关动作明确为文件或父目录删除时，才标记灰色“源已删除”并从更新、迁移、
提交清单排除。无法取得删除证据时仍按缺失源文件处理，不能自动放行。
...
目标单号是主关联键。海外路线使用 `OSCOA-*`，国内 OB 路线使用与源任务相同的
`SERIA-*`；提交说明中的源单号、源 revision 或批次哈希作为增强证据。
...
四项状态筛选旁提供资产关键词输入；匹配资产名和路径，单号视图据此反查并只保留
关联任务，资产视图保留匹配叶子及其目录层级。
后台更新和迁移不得用单行进度占位替换结果树；页签、展开状态、当前选择和滚动
位置保持可操作。短阶段状态写入受影响的任务、目录和资产行，完整说明继续由顶部
固定进度区承载。
...
- 主动作按阶段切换：未解析时突出“解析”，解析后突出“更新并复核”或“刷新状态”，
  ...
  一个信号黄主按钮。
- “更新并复核”在任何 SVN 写操作前打开待处理资产目录树；确认后只更新勾选项
  对应目录，并把该选择沿用到迁移和提交，不让未选资产重新进入当前批次。
...
- 同一资产涉及多个单号时，资产树只显示一次，右侧列出全部真实映射；不能根据独立
  的源/目标单号集合生成笛卡尔组合。
- 路线菜单包含国内 trunk → 海外 trunk、国内 trunk → 海外 trunk → 海外 OB、
  海外 trunk → 海外 OB、国内 trunk → 国内 OB 四项。
- 设置窗口按“工作区、固定表、核验策略”分组；四个工作区使用 2×2 布局、固定表
  两列并排，核验策略集中展示扫描范围、最大回溯和自动刷新。
- 国内 OB 工作区可留空；选择国内 OB 路线且本地目录不可用时，主动作显示
  “配置国内 OB”，不触发海外远端刷新。
...
8. [x] 国内 trunk → 国内 OB 的 SERIA 单阶段本地核验。
