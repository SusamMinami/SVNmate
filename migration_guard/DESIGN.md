...
- **Contextual:** “定位文件”只在本地复核的资产叶子上启用；远端资产和目录
  没有可定位的工作副本文件。
- **Scoped update:** “更新并复核”在执行 SVN update 前打开资产树；确认按钮
  显示所选数量，空选择禁用。该选择继续约束后续迁移与提交，不重复询问。

### Inputs / Fields

- 粘贴区允许网页原文和多行任务，`Ctrl+Enter` 与“解析”触发相同行为。
- 自动解析结果先进入可多选单号列表，同时后台只读查询 Jira/SVN；查询与手动刷新
  不得锁定选择。“使用选中”只确认后续写操作范围。Jira 过渡态使用中性色
  “等待 SVN”，绿色只来自最终资产提交证据。
- 解析结果在单号页签与右侧详情中展示，并在缺失或一对多映射时就地显示原因。
- 路线名称和右侧箭头打开同一个四路线菜单；海外 Trunk-OB 显示完整三段路径，
  国内 OB 显示独立单阶段路径。路径编辑只从右上角设置进入。
- 国内 OB 路线直接解析粘贴文本中的 `SERIA-*`；缺少工作区时主动作显示
  “配置国内 OB”，不显示会误导用户的“刷新状态”。
...
- 后续 SVN revision 已删除源文件或父目录时使用灰色“源已删除”，该状态表示旧单
  当前无需迁移，不借用绿色完成语义。
- 文件、目录、Jira 进度和远端资产模式必须保留明确的列标题，不能只靠行色区分。
- 结果区使用“单号 / 资产”紧凑页签；单号视图按任务聚合并可展开资产，资产视图
  按路径去重并在详情区展示全部关联单号。
- 四项状态筛选旁保留紧凑资产搜索框；按名称或路径片段过滤，单号视图仅显示
  包含匹配资产的任务，资产视图显示匹配叶子及必要目录层级。
...
- 更新和迁移期间保留“单号 / 资产”页签与结果树交互，把短阶段状态写入受影响
  行；不得用单行进度占位替换现有结果。
...
- 设置窗口按“工作区、固定表、核验策略”分组；四个工作区使用 2×2 紧凑布局，
  国内 OB 标记为可选，保持标题、主体和保存栏的固定层级。
