...
主要用户是负责国内主干、国内 OB、海外 Trunk 与海外 OB 之间内容迁移的策划、
程序和资源维护人员。项目管理员也会在没有本地工程时，仅凭 Jira 与远端 SVN
证据检查多个海外迁移单据的进度。

## Product Purpose

MigrationGuard 把 Jira 单号映射、海外三阶段远端资产进度、国内分支合入、本地
工作副本更新、UE 资源迁移、TortoiseSVN 提交和提交后复核串成可重复执行的证据
链。成功不是“打开过提交窗口”或“目录是干净的”，而是每个必需文件都有正确单号
的目标提交证据且没有遗留本地修改。
...
- 国内 trunk → 国内 OB 为独立单阶段路线，直接使用 `SERIA-*`，不依赖海外映射表。
- 优先按提交清单更新最小目录集合，异常时才回退模块根目录。
- 在 SVN 更新前选择本批次资产范围，并在核验、UE 迁移、提交及后续 OB 阶段持续
  复用该范围；未选文件不会重新进入当前流程。
...
- 未配置国内 OB 工作区时只提示配置，不复用海外 OB 地址或海外远端状态。
...
- 业务术语 `国内 trunk`、`国内 OB`、`海外 trunk`、`海外 OB`、`未迁移`、`待提交`、
  `已完成`、`需确认`、`阻断` 必须保持一致。
