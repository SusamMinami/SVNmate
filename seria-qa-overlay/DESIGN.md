...
- **Installer:** A light native WinForms surface mirrors SVNmate's Metro language: `#F3F3F3` work area, white sections, Segoe UI text, restrained borders, and a `#0067C0` primary command. It starts with an empty editable target unless a path is explicitly supplied; `粘贴` reads the clipboard, inline feedback validates the resolved Win64 directory, and browse or auto-detect only replace the value after the user invokes them. Auto-detect prioritizes the entered hint, checks its ancestors, and searches a bounded number of nearby child directories before falling back to configured/default trunk locations. Two checkboxes configure HUD visibility and whether Home opens the complete ReShade surface; a 25-100% numeric control configures opacity. `安装 / 更新` is primary; `仅校验` and `关闭` remain neutral. Update state is a compact clickable dot before the bottom-left version label instead of another action-row button: green means current, red means a newer Setup is available, blue means checking/downloading, amber means the check failed, and gray means not yet checked. Tooltip and accessible text name every state. The updater checks the fixed release channel at most once per local day without a startup modal. Clicking a red dot downloads a Setup whose URL and SHA-256 come from the validated manifest, then launches that Setup with the resolved game path; clicking any other idle state retries the check. The read-only log uses the UI font and explicit success/error status while child processes remain hidden. After installation, verification, and recovery-copy publication all succeed, a modal completion notice states the resolved target and that the game may be started. Missing package files explain that ZIP preview execution is unsupported. No wizard pages, decorative cards, or registry state.
