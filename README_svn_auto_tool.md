...
- 顶部音乐和“隐藏到托盘”改为固定尺寸图标按钮，并提供 Tooltip 与
  `Alt+M` / `Alt+H` 快捷键。
- 当前状态移动到 `SVNmate` 名称右侧，右上角只保留图标命令与“立即执行”。
- 保存配置、日志目录、使用指南、更新与退出收纳到顶部“更多”菜单。
- 工具模块行压缩为“名称 + 版本状态 + 主动作 + 更多菜单”；检查更新、重新
  选择、打开安装位置和复制路径进入行级菜单。
- 实时输出标题旁增加成功、失败和跳过计数；存在失败时不再显示绿色完成状态。
- 主题色板与 ttk 样式抽到独立模块，并在 DPI 切换后同步更新窗口最小尺寸。
- 在线升级兼容平铺和带外层目录的 ZIP，并在替换前验证主程序文件。
- 执行与自动化区将定时设置并入标题行，移除重复说明与分隔行，扩大实时日志。

## v1.4.4 更新摘要

- 正式提供 Named Pipe 外部更新接口和共享 `svnmate_core`。
- 新增迁移核验助手及独立在线更新通道。
- 迁移核验助手按单据涉及路径执行精细 SVN 更新，不再默认更新整个 trunk。
- 增加固定阶段进度条，并在 checkout 后按提交日志和本地状态完整复核。

## v1.4.3 更新摘要

- Explorer 重启后会自动重新注册托盘图标。
- 隐藏主窗口前会检查托盘图标；若恢复失败，窗口保持打开并显示原因。
- 空闲时降低日志、托盘、定时任务和 DPI 检查频率，减少后台唤醒。
...
右上角的音乐图标用于播放或暂停。默认开启，工具会优先播放 exe 所在目录下的
`.mp3` 音乐文件，也兼容 `.wav`；悬停可查看当前状态，也可按 `Alt+M` 切换。
...
顶部“更多”菜单提供保存配置、日志文件夹、使用指南、检查更新和退出；
`Ctrl+S` 可直接保存配置。点击“使用指南”会打开在线文档：
...
每行固定显示模块名称、版本状态、一个主动作和“更多”图标。主动作会根据状态
显示“安装”“打开”或“更新”；检查更新、选择现有程序、打开安装位置、复制路径
以及 Kindle 的启动联动设置位于行级菜单中。
...
执行 `Update.bat` 和 `Build.bat` 时会显示原始 CMD 窗口；工具会自动给窗口发送 Enter 来通过脚本里的 `pause`。脚本成功结束后窗口会关闭，如果脚本返回错误，窗口会保留 5 秒方便查看。全部任务结束后，实时输出标题旁会显示成功、失败和跳过数量；只有没有失败或跳过时日志区域才显示绿色完成状态。
