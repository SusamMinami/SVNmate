...
1. 点击“选择数据目录”。
2. 将“实时数据目录”指向 UE 导出的 `res` 或其他包含对话表、开始节点和任务表
   的目录。
3. 将“配置文档目录”指向包含 `csvdir`、`xlsdir` 的 `doc`；NPC、模型、
   目标物和地图 CSV 从这里读取，Excel 注册也固定写入这里。
...
桌面端会保存实时数据目录和配置文档目录。注册扫描始终读取配置目录中的
NPC、模型、目标物和地图 CSV；Excel 写入固定使用
`<配置 doc>\xlsdir`，不受实时 `res` 目录影响。
...
首次启动分别引导选择实时数据目录和配置文档目录；两处都可位于任意磁盘，
保存后下次启动自动恢复。飞书、TRAE、UE OmniMcpCore 与在线升级状态统一在
桌面版设置中管理。UE 默认地址为 `127.0.0.1:12031`。点击“生成 TRAE 配置”后，应用
