...
6. 程序先显示角色和完整对话文本，导演页提供默认收起的剧情梗概，同时等待 BP 站位查询；检测到
   ...
   BP 结果替换。
7. AI 完成后直接对比 AI 占位与当前 BP/规则占位。采用 AI 占位会进入已有
   分镜；采用当前占位会先展示这版 AI 分镜，再在后台按当前占位重新生成。
...
重新排位”预览选择。如果没有找到与开始节点同名的 BP，工具会继续读取 UE
...
当前对话与站位检查本地缓存和飞书共享库，不再重复查询 BP。精确命中时直接
显示已有方案；没有命中才提交新的模型任务。新生成的 TRAE 结果会先打开占位
对比窗口；选择 AI 占位时直接展示已有方案，选择当前占位时锁定当前坐标与朝向
重新生成。重新生成期间仍可浏览上一版 AI 分镜，也可切回规则导演，后台结果
不会丢失。当前已
处于 TRAE 模式时再次点击“TRAE 协作”会跳过本地缓存和共享库精确命中，明确
生成一套新方案。规则、缓存和新方案之间切换时保留当前镜头编号；目标方案镜头
不足时才收敛到最后一个镜头。
...
重新打开对比弹窗，确认前不覆盖当前方案。BP 选项始终显示 Blueprint 保存的
原始位置与旋转，不叠加第一个分镜规划的演员转身。
...
- **资料库试听**：音频页在推荐内容下方提供音效与音乐资料库入口。展开后先按
  ...
  准备试听文件，音乐直接使用同步的附件，缺少媒体时会显示明确原因。
