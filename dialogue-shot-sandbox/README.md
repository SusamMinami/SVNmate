...
点击后可查看实时目录、配置目录、目标物表路径、当前数据量和返修案例库状态，
并可控制案例收集、刷新案例库或直接进入设置。
...
模型。发言 NPC 缺少对应模型或显式 `AM_Talk` 槽位与模型冲突时，程序会说明
原因并跳过该 BP。
...
任务目标物工作区的“对话文件 ID”默认收起到 BP 文件名右侧；只有需要覆盖从
BP 名称自动推导的对话 ID 时才展开填写，折叠后仍会显示当前已填 ID。
