...
选择完整 `doc` 目录后，点击顶部“任务目标物”，输入 `Mission.id` 可解析
`Mission.ShowNPC` 引用的全部目标物，并查看目标物、NPC、模型资源、地图和
世界坐标、Pitch/Yaw/Roll 旋转关系。目标物默认全选，可逐项取消；确认后
只将已勾选的实际模型或定位标记加载到 UE 当前关卡。弹窗内容区域独立滚动，
底部清除和加载按钮始终可见。

所有目标物必须配置相同且有效的 `MissionPosition.MapID`，否则在调用 UE
之前停止加载并列出冲突项。当前关卡与目标地图不同时，工具通过
`MapConfig.resourceid -> Scene.path` 自动打开对应关卡；当前关卡存在未保存
修改时不会自动切图。预览对象不会修改 Excel 或 CSV，使用完应通过弹窗中的
“清除预览”移除。详细规则见
[`docs/mission-target-preview.md`](docs/mission-target-preview.md)。

同一窗口也可将已勾选且具有实际模型资源的目标物写入用户预先创建的空镜头
