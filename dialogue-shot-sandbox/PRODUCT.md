...
它不是通用 3D 场景编辑器或文本生成器。产品把带可选端侧顾问的规则导演、内部 TRAE 协作、Mira AI、
...
- 规则导演先用可替换端侧模型分析叙事节拍，再生成合法镜头并离屏渲染体型代理
  候选画面。几何系统为每个镜头提供多个合法机位，模型逐镜逐图评分排序，最终
  组合仍需通过关系轴与投影验收；模型不可直接输出或绕过摄影机坐标。
- 规则导演、TRAE 和 Mira 均需通过 `shot-plan.v5` 与投影验收。
- BP 体型来自 Mesh 包围盒和组件缩放，预览与机位共用体型；眼肩关键点当前仍为
  比例估算。验收区分硬性失败与构图建议，支持普通关系镜头持续停留。
- 场景参考支持只读落点确认、附近静态网格/实例包围盒、沙盘线框与机位粗筛，
  不是实时场景；AI 分享默认关闭，应用时显式重算规则镜头。
- UE 场景截图、深度验收和独立无对白空镜尚未接入；当前视觉评分基于角色代理，
  不能视作真实关卡渲染器。
- 真实 CSV 必须在 Worker 中分块解析；Three.js 视口按需渲染。
- 配置小窗按当前 UE 节点提供镜头、音频和动作编辑；轻量监听与精确反射分频，
  不以高频完整属性回读换取表面实时性。
- 任务目标物可将 `SceneObject` 或直接 `SeriaNPC` 实例复用既有数字槽和
  `DialogModels` 管线写入 BP；同模型多实例保留为不同角色槽。
