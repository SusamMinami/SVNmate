...
- 用户同时处理分镜工作台、NPC 注册、任务目标物、NPC 迁移和动画语音五个
  常驻工作区。
...
- 规则导演立即显示本地基线，端侧节拍完成后先更新方案，再离屏渲染体型代理
  候选画面。几何系统为每个镜头提供多个合法机位，模型逐镜逐图评分排序，最终
  组合仍需通过关系轴与投影验收；模型不可直接输出或绕过摄影机坐标。
  配乐在镜头就绪后独立分析；可停止分析并保留当前方案，编辑后不被晚到结果覆盖。
- 规则导演、TRAE 和 Mira 均需通过 `shot-plan.v5` 与投影验收。
- 生成方案可交给 TRAE 精修当前镜头：固定当前基线，携带相邻镜头和已有反馈，
  预览后采纳到本地草稿；旧版本与越界回传不覆盖现有镜头、站位和动作。
...
- 配置小窗按当前 UE 节点提供镜头、音频和动作编辑；识别 `00` 配置节点后可按
  职业配置表选择预览角色并安全写入 `PreviewSchoolID`。轻量监听与精确反射分频，
  不以高频完整属性回读换取表面实时性。
- 动画语音工作区按用户指定目录扫描 LevelSequence，审核字幕段、`skip` 标记和
  可唯一确认的 Show/Hide 事件时间；写入后只标脏，不自动保存 UE 资产。
- 只有对白、尚未生成分镜时仍可按节点使用导演、镜头、音频和 UE 检查器；音频
  与角色动作不依赖分镜存在即可编辑和导出。
- 端侧顾问只从真实音乐目录中提出有明确叙事依据的节点级配乐建议，并结合 UE
  当前状态抑制重复建议；没有可靠匹配时保持为空。
