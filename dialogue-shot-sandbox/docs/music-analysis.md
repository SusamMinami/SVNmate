...
> 文档状态：现行专题规范
>
> 最近核对：2026-09-09，对应镜头沙盘 `0.24.4`。全库分析只由显式
> `npm run analyze:music` 触发，普通设置同步只读取目录元数据和已发布分析结果。
...
只有资源标识能唯一映射到当前 CSV 状态 ID 的音乐才进入推荐目录；同步结果区分
未映射与缺附件。目录同步只读元数据与已发布分析，试听附件按需下载，以
`fileToken + 文件名` 缓存，保留原扩展名并支持 HTTP Range。
分析结果只有 token 与当前附件一致时才参与推荐，不复用旧附件的特征。
...
音乐推荐由规则导演的端侧顾问综合剧情语义、人工标签、备注、音频摘要和 UE
当前配乐后生成。顾问只能引用已同步目录中的状态 ID，并绑定当前对白节点；
普通信息交流、没有可靠匹配或现有配乐已经合适时保持空结果。音频特征只提供
同类候选依据，不允许低置信 BPM 或单一频谱指标覆盖叙事判断。

应用会过滤重复节点、目录外状态和与 UE 当前状态相同的建议。音频页只在建议
所属节点展示新增或替换项；已有音乐名称、状态 ID 与延迟由“UE 当前音频”区域
独立呈现，不再把上一节点配乐显示为当前节点的“沿用中”建议。
