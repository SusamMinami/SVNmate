...
> 文档状态：现行规则说明
>
> 最近核对：2026-09-06。机器可读规则集
> `shot-language-rules.v1.json` 当前版本为 `1.7.0`；`shot-plan.v5` 是分镜输出
> Schema，二者版本含义不同。
...
运行时通过 `projection.issues` 输出 `ruleId/severity/message`。只有 `error`
使 `projection.valid=false` 并进入一次返修；`warning` 显示“有建议”，`info`
说明已经完成的标签修正。旧方案没有 issues 时保留原有效性，不静默消除历史错误。
JSON 是规则政策目录，不是动态表达式执行引擎；已落地检查以
`shotGeometry.ts`、`shotResolver.ts` 为准，不能把目录里的所有 machine_check
都宣称为已经实现。

### 体型与真实空间

- `participants.body_profile` 同时进入本地、AI 与共享预览。尺寸单位是米，
  `footOffset` 是角色朝向坐标系中的右、上、前偏移，不能与槽位原点混用。
- Formation 槽位 Z 不再归零；Mesh 组件链的偏移、旋转和缩放参与包围盒估算，
  槽位缩放仅应用一次。身体关键点当前按身高比例估算，不是实测骨骼姿态。
- 个体近景使用自身关键点；关系全景使用全体垂直范围，并分别检查主要角色
  21:9 安全区。严重头部投影重叠按体型代理检测，不代表场景三角网格遮挡验证。
- 群像不再触发全员面向群体中心的转身。必要对话转身继续尊重 `can_turn`，
  不可执行时可在一次返修中改用观察式全景，不篡改 BP 朝向。
- 构图求解先减少硬性失败，再比较构图偏好；群像尝试有限的方位与距离候选。
- 缓存输入包含体型和 `geometry_version`。更新体型或计算策略后，旧精确缓存
  不会被当成相同输入直接复用。
...
采用 Formation BP 时，`facingTarget` 必须由各数字槽的
`RelativeRotation.Yaw` 转换，不能直接替换成当前对话对象。若交流关系要求
角色改变视线，只能规划项目现有的左右 45°、90°、180° 转身动作；转身后的
朝向由后续镜头继承。不可转身角色应调整摄影机位置和构图，不得在预览中假装
角色已经转身。
...
- 静态机位是普通对话默认值。镜头运动必须对应信息揭示、角色移动、情绪
  推进、关系变化或空间重建。
- 规则导演不把感叹号、强调词或节拍强度直接映射成推近和压迫留白；
  端侧建议需要具体视觉动机及当前镜头内的原文依据，协议见
  [端侧顾问](rule-director-edge-advisor.md#节拍协议)。
...
- 普通镜头偏好 4-8 秒和多句台词，不是强制时长。相同静态关系覆盖可延续，
  当前规则导演在不跨进出场、反应或覆盖需求变化的前提下合并至最多 24 秒。
- 节拍不等于切镜；相邻节拍可以共用一个镜头，一个节拍也可以跨多个镜头。
  叙事标签与强度变化不自动切镜，合并后保留各节拍意图摘要。
- 切镜由信息、情绪、动作、权力或空间变化驱动，不由说话人变化单独触发。
- 多句台词分段后，再决定镜头应拍说话者、听者反应还是双人关系。
- 角色进出场、移动、轴线改变或连续三个紧景后，评估是否需要重新建立空间。
- 普通问号和同人连续发言不自动触发特写推近；不为避免重复主体而强制切到
  另一角色。停顿、明确逼问、信息揭示与行动提示仍可构成独立节拍。
- 30°只检查同主体且景别变化不足的切点，使用绕主体的机位方位而不是光轴差；
  不再将这项软建议当作所有相邻镜头的失败条件。
...
1. 按进出场、时长和叙事覆盖需求划分镜头段，不将每个节拍起点直接当作切点。
...
场景参考和无对白空镜的分阶段接入见
[`scene-composition-integration.md`](scene-composition-integration.md)。
