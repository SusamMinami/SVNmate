...
2. 角色列表优先复用动作编辑器已经读取的模型槽和角色名。机位 Transform 仍从
   ...
   识别预览实例，多个同等候选才阻断。预览实例缩放必须为 `1 / 1 / 1`。
3. 机位选择使用紧凑俯视图：NPC 圆球位于中心，数字相机组件按相对 NPC 的真实
   水平方位分布在外圈；NPC 面向屏幕下方，相机位于其正面一侧。点击相机图标
   完成选择。界面不常驻显示坐标或角度数值，但保留按钮的可访问名称和悬停说明。
   不为候选创建 SceneCapture 或 WebGL 预览；读取角色名不再调用
   `SeriaDialogCharacter.get_dialog_actor()`。
