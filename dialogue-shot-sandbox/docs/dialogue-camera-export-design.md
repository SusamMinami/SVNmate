...
6. 全量页面中每个镜头可独立勾选；同一镜头覆盖的起点和延续节点作为一个整体处理，
   未勾选镜头不写入也不清空。
7. 对话资产或 Formation BP 已有未保存修改时阻断导出，避免保存无关改动或
   使用随后可能被撤销的站位。
8. 投影验收失败只显示警告，不阻断用户确认。
9. 用户勾选覆盖确认后，才能点击“确认写入并保存”。
10. 服务端对勾选镜头重新执行预检；若 UE 数据已变化，审核令牌失效并要求
   重新检查。
11. 写入后逐节点回读；结构、枚举和布尔值严格匹配，浮点数按 UE4 float32
    精度容差校验，全部一致后保存对话资产。
