...
播放状态由分镜工作区统一持有。切换导演、镜头、音频或 UE 页签不会停止当前
试听，暂停按钮直接显示在音频页签；切换到 NPC 注册、任务目标物或 NPC 迁移等
一级工作区时立即停止。播放中的资源只改变所在行背景，不增加侧边色条。

资料库资源只有在成功开始试听后才显示应用按钮；再次点击已应用按钮会移除该
节点上的音效选择，并恢复为可应用状态。音频页的“UE 当前音频”区域统一显示
当前节点已有音效、延迟、配乐名称、状态 ID 和延迟；待写入建议单独显示。只有
对白、尚未生成分镜时仍可按当前节点试听、选择并进入音频导出预检。
