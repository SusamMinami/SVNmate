...
输入四位对话 ID（例如 `7352`）时，会自动展开为 `BP_735200` 后搜索。
可选的对话文件 ID 默认收起在 BP 文件名右侧，仅在需要覆盖自动推导结果时
展开填写。收起后只保留固定尺寸的图标按钮；存在覆盖值时显示状态点，并在
悬停提示中给出实际 ID。修改 BP 文件名会清空旧覆盖值，避免静默写入错误对话。

未填写对话文件 ID 时，“加载 BP 到 UE”直接读取数字槽并按 BP 原始位置加载，
不要求先执行对话调度预览。填写对话文件 ID 后，加载改为两个明确步骤：

1. “计算最终站位”沿完整 `NextID` 链读取所有节点，包括关闭 UI 或没有对白的
   节点。
2. 每个节点先应用 `Dialog.RelativeTransformsString`，再按顺序执行
   `ERotate`、`EWalk` 和 `EStateMachineWalk`；走位终点采用动作中的 UE
   局部坐标，朝向采用明确的节点旋转或移动方向。
3. 俯视图绘制 BP 原位、位移路径、最终位置和最终朝向，并逐槽显示最终世界
   坐标、位移距离及动作次数。
4. 用户确认后点击“加载最终站位”。工具将各槽最终 Transform 换算到
   Preview Level 的世界坐标并生成 editor-only Actor。
