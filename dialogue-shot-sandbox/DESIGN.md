...
- 按轨道顺序上下切换，300ms 整页同时进出，快速反向或跨页从当前位移接续，
  所有尚在退出的页面完成后才隐藏；中性页边阴影，合成提示仅在转场期间启用。
  键盘/辅助激活与减少动态效果即时切换，运行中开启减少动态效果立即归位。
  不使用扫线、黄分隔或短距离假位移。新读取/新任务才按操作语义刷新旧状态。
- 设置、授权、冲突和高风险确认用模态框；长表与连续编辑使用专用窗口。
- 状态弹层点击外部或 Escape 关闭，不放额外关闭按钮；首次占位选择和首次设置
  则必须允许显式关闭。不能混用两种规则。
  键盘打开时焦点进入弹层，Escape 返回触发按钮；点击外部保留目标焦点，
  Tab 离开弹层及其触发按钮时关闭。收起即不可交互，退出画面可短暂保留。
...
| NPC 迁移 | 顶栏已显示工作区名称，内部不再预留重复子标题带；三个等宽大图标入口；子流程仅保留网格返回，不并排另放返回分镜箭头；全新迁移在生成计划后自动收起源资产与迁移参数，类型选择保持主位、低频参数进入高级配置；阻断、提醒和最终复核归入左侧对应步骤，待处理项标红置顶；动作表不重复显示清单标题栏，首列表头用三态复选框批量选择，“动作 / 修改时间”表头内切换名称或时间排序 |
...
小窗预设机位使用俯视轨道：NPC 圆球固定居中并面向屏幕下方，相机图标按真实
角色正面相对方位排列；点击图标选择，不在图面堆叠坐标或角度数字，键盘焦点与
选中态必须清晰可辨。混合曲线资产仅在选择“添加镜头曲线”后的审核区编辑。
...
- 导演分段按钮 hover 与侧栏反馈一致：未选项转为深灰底白字，不显示青色底线；
  已选项保持黑底白字，持久状态不靠 hover 改写。
...
- 按下 80ms 缩放到 98%，不改布局尺寸；禁用不播放 hover。图标有 `title`
  或自定义说明与可访问名称，进度/成功/失败不只靠颜色。
  顶栏状态按钮复用按压反馈；键盘和减少动态效果取消壳层按钮缩放。
- 顶栏状态弹层从右上触发点以 0.98→1、上移 3px→归位展开，160ms 入场、
  100ms 退出；快速开关从当前状态反转，键盘与减少动态效果即时显隐。
  顶栏自定义说明首次悬停等待 350ms、125ms 淡入，已读说明后的 600ms 内
  邻接查看即时显示；键盘焦点即时说明，Escape 可关闭，避免叠加原生 title。
- UE 长读取显示具体阶段；地图仍在加载时不能提前报失败。

### 镜头列表与检查器

- 固定高度镜头行，选中黑底与文字即时同步，避免移动背景造成短暂低对比度。
  仅左侧共享状态条以 160ms ease-out 移动；键盘/辅助激活与减少动态效果时即时定位。
  普通 hover 不移动编号或整行；工作台键盘/减少动态效果不使用按钮按压缩放。
...
- 切镜保留页签；切镜、切节点与切页签立即显示内容，不重播裁切或位移动画。
  梗概默认收起，同对话保留展开状态，新对话重置。
...
| 轨道展开 | 300ms 宽度/标签透明度 |
| 镜头列表选中 | 黑底/文字即时切换；仅状态条 160ms，键盘与减少动态效果时即时 |
| 检查器页签 | 即时显示，无裁切或位移 |
| 镜头/视图切换 | 直接显示真实画面，无揭幕遮罩，不改变相机自身运镜 |
| 顶栏状态弹层 | 160ms 入场 / 100ms 退出，可中断；键盘与减少动态效果即时 |
| 顶栏 tooltip | 首次延迟 350ms、淡入 125ms；600ms 连续浏览窗口内即时 |
| 主工作区切换 | 300ms 整页进出，从当前位移接续；键盘与减少动态效果即时 |
