...
- 四位 ID 只加载本地对白；“读取 BP 站位”和导演入口是显式下一步。
  BP 读取入口归入左栏“占位方案”状态行，用紧凑行内按钮承载读取/重读；
  不再在搜索框下方独占整行。读取后的当前方案、刷新与切换继续共用该状态行。
...
- 规则基线、节拍方案和视觉选优依次在同一工作台更新；配乐等待不遮挡已就绪镜头。
  “停止分析”位于左侧进度文字下方，停止后保留当前结果并移除忙碌状态；再次点击
  规则导演可重新分析。普通切镜不停止，编辑数据或切换工作区会停止后台端侧任务。
- 已有 UE 镜头的显式评估入口位于左侧导演区下方，评估进度、停止、重试与视觉均分均归左侧。
  右侧导演检查器展示逐镜分数、规则问题和模型建议，不重复任务控制与均分。
  评估区局部 warning 使用 `#8a5612`、error 使用 `#ad3c30`，在 paper 上对比度分别为 5.48:1、5.41:1；
  同时保留“建议复核”与“未通过”文字标签，不只靠颜色区分状态，不改全局 token。
- 已有分镜的调整建议由用户显式生成；进度与“停止生成建议”归左侧，右侧导演检查器
  展示调整理由、同轮原镜头与建议方案的评分对比，以及预览、采纳与保留操作。
  原镜头与建议方案在中央视口即时切换，未采纳预览明确标注“建议预览 · 尚未采纳”。
  “采纳到草稿”只更新本地镜头，“保留原镜头”结束建议预览；“撤销本次采纳”恢复
  采纳前的本地镜头。写入 UE 仍需单独显式导出并确认，代理评分不代替 UE 场景与动画验收。
- 生成建议、采纳、保留与撤销的同步操作在触发按钮消失前，将焦点移至稳定的
  “分镜调整建议”区，不滚动页面；异步结果与普通切镜不抢焦点。
  停止生成建议后，焦点返回左侧稳定的“已有分镜评估”区。
- 右侧导演检查器的“TRAE 局部精修”区提供“精修要求”输入与“精修当前镜头”入口，
  只调整当前镜头，保留对白覆盖、站位与动作。局部操作使用紧凑、可换行的中性按钮，
  启用态文字与边框清晰可辨，不呈现禁用外观，不新增全局 token。
- 精修就绪后展示理由、当前与精修镜头的标签及焦距对比，提供“预览精修镜头”/
  “返回当前镜头”切换、“采纳精修到草稿”与“保留当前镜头”。中央视口明确标记
  未采纳的精修预览；采纳只更新本地草稿，尚未写入 UE。
- 处理中提供“停止精修”，停止后提示“已停止精修，保留当前方案。”；基线变化时
  清除旧结果并提示“当前方案已变化，旧精修结果已失效。可基于当前镜头重新提交。”
- 停止、采纳与保留操作在触发按钮消失前，将焦点移至稳定的“TRAE 局部精修”区，
  不滚动页面；异步完成只更新结果与状态文字，不抢焦点。
- 完整窗口动作按台词节点折叠，支持角色、多动作、延迟与拖拽排序；
  候选菜单连续滚动、同时约 8 行，空白项不提交，既有动作只读。
- 小窗不重复节点折叠栏，视线在动作上方，两栏独立收纳并保留草稿；
  UE 既有动作默认灰色锁定，点击左侧锁后以白底开放延迟编辑与同轨拖拽排序，
  未展示的特殊动作继续原位保留；角色行回退按钮撤销整轨调整。
